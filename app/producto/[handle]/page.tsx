      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-8">
        <div className="grid gap-10 lg:grid-cols-2">
          <div
            className={
              product.imageUrl
                ? "aspect-square rounded-sm bg-white p-4 flex items-center justify-center overflow-hidden border border-gold-400/25"
                : "aspect-square rounded-sm bg-paper-soft flex items-center justify-center overflow-hidden border border-gold-400/25"
            }
          >
            {product.imageUrl ? (
              ...
              <span className="font-display text-6xl text-gold-400/50">{product.title.charAt(0)}</span>
            )}
          </div>
          <div>
            <Badge variant="outline" className="mb-4">{product.category}</Badge>
            <h1 className="font-display text-3xl text-ink mb-3">{product.title}</h1>
            ...
            <p className="mt-8">
              <Link
                href={`/tienda/${product.department}`}
                className="text-sm text-ink-60 underline hover:text-gold-400"
              >
