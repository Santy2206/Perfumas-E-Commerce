...
export function SearchSuggestInput({
  ...
  return (
    <div ref={rootRef} className={cn("relative", className)}>
      {withIcon && (
        <svg
          className="pointer-events-none absolute left-4 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-bone-60"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          aria-hidden
      ...
      <input
        ...
        className={cn(
          "w-full rounded-sm border border-gold-400/30 bg-white/5 text-sm text-bone placeholder:text-bone-60 focus:outline-none focus:ring-2 focus:ring-gold-400",
      ...
      {show && (
        <ul
          id={listId}
          role="listbox"
          className="absolute left-0 right-0 z-40 mt-1 max-h-64 overflow-auto rounded-sm border border-gold-400/30 bg-wine-950 py-1 shadow-lg"
        >
          {suggestions.map((s, i) => (
            <li key={s.id} role="option" aria-selected={i === active}>
              <button
                ...
                onClick={() => pick(s)}
              >
                <span className="font-medium text-bone">{s.label}</span>
                {s.secondary && (
